...
FRAME_CT = "e0f2ae2cf9937cb0c87847f9b6d5bb49a97f5ce2360d619fbe3f5cba1b5207"
FRAME_TAG = "8b5ba8e7996a86f964c53d32f3570696"
PLAINTEXT = b'{"n":13,"s":12,"t":230,"h":610}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([13]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, b"\x0d", PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, b"\x0d", ciphertext, tag)
