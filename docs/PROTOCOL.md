...
```
{"n":13,"s":12,"t":235,"h":610}
```

where `n` is the node id, `s` is the monotonic sequence number, `t` is the
temperature in tenths of a degree Celsius, and `h` is the humidity in tenths of
a percent.
